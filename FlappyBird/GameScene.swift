...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            print("アイテムゲット！")
            // 効果音を鳴らす
            let seAction = SKAction.playSoundFileNamed("button16.mp3", waitForCompletion: false)
            runAction(seAction)
    ...
    func setupScoreLabel() {
        ...
        itemScoreLabelNode.fontColor = UIColor.redColor()
    ...
    func setupItem() {
        // アイテムの画像を読み込む
        let itemTexture = SKTexture(imageNamed: "eclipse32")
        ...
        let createItemAnimation = SKAction.runBlock({
            ...
            let itemNodeMid = SKNode()
            itemNodeMid.position = CGPoint(x: self.frame.size.width + itemTexture.size().width * 2, y: 0.0)
            itemNodeMid.zPosition = -60.0 // 雲より手前、壁より奥
            ...
            item.position = CGPoint(x: 0.0, y: under_wall_y)
            itemNodeMid.addChild(item)
            ...
            self.itemNode.addChild(itemNodeMid)
        })
